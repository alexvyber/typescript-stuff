...
type Result = Exclude<Fruit, 'pear'> // 'apple' | 'banana' | 'orange' 2. Remove multiple members from a union
...
type ClickAndFocusEvent = Exclude<Event, 'change' | 'abort' | 'blur'> // 'click' | 'focus' 3. Remove a member of a discriminated union
type Event =
| {
type: 'click'
}
...
type ClickAndFocusEvent = Exclude<Event, {type: 'change' | 'abort'}> // { type: 'click' } | { type: 'focus' } 5. Exclude members of a discriminated union by shape
type Routes =
| {
route: '/user'
search: {
